...
Pour un {term}`raccordement` triphasé **avec** neutre, il est possible de ponter le neutre entre chacune des 3 sous-alimentations.  
...
```{warning}
Pour un {term}`raccordement` triphasé **sans** neutre, essentiellement le 3 * 230 V en **Belgique**, il ne faudra pas souder ces ponts.
```
...
```{important}
Lors du montage de ces composants, il ne doit y avoir aucun espace entre la base du transformateur et le {term}`PCB`.
```
